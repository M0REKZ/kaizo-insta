...
int CCollision::GetZoneHandle(const char* pName)
{
	...
	char aLayerName[12];
	for(int l = 0; l < m_pLayers->NumLayers(); l++)
	{
		CMapItemLayer *pLayer = m_pLayers->GetLayer(l);
		
		if(pLayer->m_Type == LAYERTYPE_TILES)
		{
			CMapItemLayerTilemap *pTLayer = (CMapItemLayerTilemap *)pLayer;
			IntsToStr(pTLayer->m_aName, sizeof(aLayerName)/sizeof(int), aLayerName, std::size(aLayerName));
		...
		else if(pLayer->m_Type == LAYERTYPE_QUADS)
		{
			...
			IntsToStr(pQLayer->m_aName, sizeof(aLayerName)/sizeof(int), aLayerName, std::size(aLayerName));
			if(str_comp(pName, aLayerName) == 0)
				LayerList.add(l);
		}
	}
...
void CCollision::GetAnimationTransform(float GlobalTime, int Env, CLayers* pLayers, vec2& Position, float& Angle)
{
	CEnvPoint *pPoints = 0;
