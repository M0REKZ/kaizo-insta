...
void CGameControllerLMS::OnPlayerConnect(class CPlayer *pPlayer)
{
...
int CGameControllerLMS::OnCharacterDeath(class CCharacter *pVictim, class CPlayer *pKiller, int WeaponId)
{
...
void CGameControllerLMS::SetAllUndead()
{
    for(int i = 0; i < MAX_CLIENTS; ++i)
    {
        if(GameServer()->m_apPlayers[i])
...
void CGameControllerLMS::FakeEndRound()
{
...
void CGameControllerLMS::KillEveryone()
{
    for(int i = 0; i < MAX_CLIENTS; ++i)
    {
        if(GameServer()->m_apPlayers[i])
...
bool CGameControllerLMS::CanJoinTeam(int Team, int NotThisId, char *pErrorReason, int ErrorReasonSize)
{
