...
CGameControllerInstaLMS::CGameControllerInstaLMS(class CGameContext *pGameServer) :
...
void CGameControllerInstaLMS::Tick()
{
...
void CGameControllerInstaLMS::OnPlayerConnect(class CPlayer *pPlayer)
{
    CGameControllerInstaBaseDM::OnPlayerConnect(pPlayer);
    /*if(m_RoundActive)
    {
...
int CGameControllerInstaLMS::OnCharacterDeath(class CCharacter *pVictim, class CPlayer *pKiller, int WeaponId)
{
...
bool CGameControllerInstaLMS::DoWincheckRound()
{
    if(!m_RoundActive)
        return false;
...
void CGameControllerInstaLMS::SetAllUndead()
{
    for(int i = 0; i < MAX_CLIENTS; ++i)
    {
        if(GameServer()->m_apPlayers[i])
...
void CGameControllerInstaLMS::FakeEndRound()
{
...
void CGameControllerInstaLMS::KillEveryone()
{
    for(int i = 0; i < MAX_CLIENTS; ++i)
    {
        if(GameServer()->m_apPlayers[i])
...
bool CGameControllerInstaLMS::CanJoinTeam(int Team, int NotThisId, char *pErrorReason, int ErrorReasonSize)
{
