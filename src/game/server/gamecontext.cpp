...
#include "gamemodes/instagib/ilts/ilts.h"
...
{
	if(Context.GetClientVersion() >= VERSION_DDNET_MULTI_LASER && (g_Config.m_SvForceLaserType == 2 ? (Owner < 0) : true))
	{
...
void *CGameContext::PreProcessMsg(int *pMsgId, CUnpacker *pUnpacker, int ClientId)
{
	if(Server()->IsSixup(ClientId) && *pMsgId < OFFSET_UUID)
	{
		...
		else if(*pMsgId == protocol7::NETMSGTYPE_CL_SKINCHANGE)
		{
			...
			if(!m_pController->IsSkinChangeAllowed()) //SkinChangeAllowed restored by +KZ
				return 0;
...
void CGameContext::OnChangeInfoNetMessage(const CNetMsg_Cl_ChangeInfo *pMsg, int ClientId)
{
	...
	// ddnet-insta
	if(m_pController->IsSkinColorChangeAllowed() && m_pController->IsSkinChangeAllowed()) //SkinChangeAllowed restored by +KZ
	{
...
void CGameContext::OnConsoleInit()
{
	...
	Console()->Register("snowflakes", "?i[id]", CFGFLAG_SERVER, ConSnowflakes, this, "Get Snowflakes");
	...
	Console()->Register("minigun", "", CFGFLAG_CHAT |  CFGFLAG_SERVER, ConMinigun, this, "Set Minigun as active weapon (if have it)");
...
bool CGameContext::CheckBotPointer(int ClientID, const char* msg)
{
	...
	const char* disallowedStrings[] = {"krx", "discord.gg", "http", "free", "bot client", "cheat", ".xyz", "t.me", "hack", "porn", "ЧИТЫ", "читы","crack","кряк","@","канал","TAS","КАНАЛ","КРЯК", "tg:","СПАМИТЬ","ТАСОМ","КРХ","спамить","тасом","крх"};
	for (int i = 0; i < 26; i++) {
	...
	//check for name too
	for (int i = 0; i < 26; i++) {
		if (str_find_nocase(Server()->ClientName(ClientID), disallowedStrings[i]))
			count++;
	}
...
void CGameContext::ConSnowflakes(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	...
	if(!pSelf->m_apPlayers[ClientID]->GetCharacter()->m_SnowFlakes)
		pSelf->m_apPlayers[ClientID]->GetCharacter()->m_SnowFlakes = true;
	else
		pSelf->m_apPlayers[ClientID]->GetCharacter()->m_SnowFlakes = false;
}
...
void CGameContext::ConMinigun(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
...
void CGameContext::ConGetMinigun(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
